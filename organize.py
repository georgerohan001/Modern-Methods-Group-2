...
def main() -> None:
    ...
    if not pngs:
        print("⚠️  No PNG files found in obj_train_data – nothing to move.")
    else:
        for p in pngs:
    ...
    if n_images == 0:
        print("❌ No images found in the newly created 'images' folder – abort.")
        sys.exit(1)

    print(f"🖼️  {n_images} PNG images will be processed (index‑gray into channel3).")
    ...
    print("✅ channel1 populated (first deletion & renumber).")
    ...
    print("✅ channel2 populated (second deletion & renumber).")

    # ------------------------------------------------------------------
    # (g)  Fix obj.names order and rewrite all annotation files
    # ------------------------------------------------------------------
    current_names = read_class_names(OBJ_NAMES_FILE)
    print(f"📄 Current obj.names ({len(current_names)} lines): {current_names}")
    ...
    if current_names == TARGET_CLASS_ORDER:
        print("✅ obj.names already in the desired order – no changes needed.")
    else:
        ...
        print("🔁 obj.names reordered and all annotation files updated.")
...
if __name__ == "__main__":
    try:
        ...
        sys.stderr.write("❗ This script needs numpy (for the index‑gray image).\n")
        sys.exit(1)
